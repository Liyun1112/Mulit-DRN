...
class Model(nn.Module):
    def __init__(self, opt, ckp):
        ...
        self.model = mdrn.make_model(opt).to(self.device)
        self.dual_models = []
        ...
        if not opt.test_only:
            print(self.model, file=ckp.log_file)  # 将模型的结果打印到日志文件中
            print(self.dual_models, file=ckp.log_file)
        
        # compute parameter
        num_parameter = self.count_parameters(self.model)
        ckp.write_log(f"The number of parameters is {num_parameter*1024 / 1000 ** 2:.2f}KB")
