...
class DRN(nn.Module):
    def __init__(self, opt, conv=common.default_conv):
        ...
        up_body_blocks = [[
            MRCAB(conv, n_feats * pow(2, p), kernel_size, act=act)
        ...
        self.conv = [
            conv(n_feats * pow(2, p + 1), n_feats * pow(2, p), kernel_size=1
                 ) for p in range(self.phase)
        ]
    ...
    def forward(self, x):
        ...
        for idx in range(self.phase):
            if len(back_layers) != 0:
                x = torch.cat((x, back_layers[idx - 1]), 1)
            else:
                x = torch.cat((x, x), 1)
            x = self.conv[idx](x)
            copies.append(x)
        ...
        for idx in range(self.phase):
            ...
            x = torch.cat((x, copies[self.phase - idx - 1]), 1)  # 下采样倒序存储了特征图，最后的1是表示在通道维度上拼接
            # output sr imgs
